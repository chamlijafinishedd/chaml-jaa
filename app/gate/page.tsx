  ...
  return (
    <main className="min-h-screen bg-slate-50 px-4 py-8 text-slate-900 sm:px-6">
      <div className="mx-auto max-w-lg rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm sm:p-8">
        ...
        {result?.booking && <div className="mt-6 rounded-2xl border border-emerald-200 bg-emerald-50 p-4"><div className="text-sm font-black text-emerald-950">{result.checkInCompleted ? "✓ CHECK-IN SUCCESSFUL" : !result.paymentConfirmed ? "⚠ PAYMENT NOT CONFIRMED" : !result.bookingConfirmed ? "⚠ BOOKING NOT CONFIRMED" : result.dateStatus === "today" ? "✓ CHECK-IN APPROVED" : result.dateStatus === "future" ? "✓ RESERVATION VERIFIED" : "⚠ RESERVATION EXPIRED"}</div><div className="mt-3 space-y-1 text-sm"><div>Customer: {result.booking.customer_name || "—"}</div><div>Booking: {result.booking.reservation_code || result.booking.id}</div><div>Date: {result.booking.booking_date}</div><div>Arrival: {result.booking.booking_time || "—"}</div><div>Adults: {result.booking.adults ?? 0}</div><div>Children: {(result.booking.children_3_plus ?? 0) + (result.booking.children_under_3 ?? 0)}</div><div>Picnic Area: {result.booking.area_name || "No Picnic Area"}</div><div>Payment Status: {result.booking.payment_status || "—"}</div><div>Booking Status: {result.booking.booking_status || "—"}</div></div>{result.checkInCompleted ? <div className="mt-4 rounded-xl border border-emerald-300 bg-white p-4 font-bold text-emerald-800">Reservation: {result.booking.reservation_code || result.booking.id}<br />Checked in at: {result.booking.checked_in_at ? new Date(result.booking.checked_in_at).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }) : "—"}</div> : result.booking.checked_in ? <div className="mt-4 rounded-xl border border-amber-300 bg-amber-50 p-4 font-bold text-amber-800">ALREADY CHECKED IN<br />This reservation has already been used for entry.{result.booking.checked_in_at ? ` · ${result.booking.checked_in_at}` : ""}</div> : !result.paymentConfirmed ? <div className="mt-4 text-sm font-semibold text-amber-800">{result.paymentError || "Payment has not been confirmed by Chamlija staff. Entry is not available until payment is approved."}</div> : !result.bookingConfirmed ? <div className="mt-4 text-sm font-semibold text-amber-800">Booking has not been confirmed.</div> : result.dateStatus === "future" ? <div className="mt-4 text-sm font-semibold text-emerald-800">This reservation is confirmed for {result.booking.booking_date}. Check-in will be available on the reservation date.</div> : result.dateStatus === "past" ? <div className="mt-4 text-sm font-semibold text-amber-800">This reservation date has passed. Check-in is no longer available.</div> : result.checkInEligible ? <button type="button" onClick={checkIn} className="mt-5 min-h-12 w-full rounded-xl bg-emerald-700 px-4 py-3 font-semibold text-white">ALLOW ENTRY</button> : null}</div>}
      </div>
    </main>
  );
}
